...
CompanyDetailsReviewOverview.propTypes = {
  company: PropTypes.object,
  onOpen: PropTypes.func,
};

export default function CompanyDetailsReviewOverview({ company, onOpen }) {
      ...
      <GridStyle item xs={12} md={4}>
        <Stack spacing={1.5} sx={{ width: 1 }}>
          {ratings
            ...
            .reverse()
            .map((rating) => (
              <ProgressCompany key={rating.name} star={rating} total={total} />
            ))}
        </Stack>
      </GridStyle>
...
ProgressCompany.propTypes = {
  star: PropTypes.object,
  total: PropTypes.number,
};

function ProgressCompany({ star, total }) {
  const { name, starCount, reviewCount } = star;
  return (
    <Stack direction="row" alignCompanys="center" spacing={1.5}>
