...
LectureDetailsReviewOverview.propTypes = {
  lecture: PropTypes.object,
  onOpen: PropTypes.func,
};

export default function LectureDetailsReviewOverview({ lecture, onOpen }) {
      ...
      <GridStyle lecture xs={12} md={4}>
        <Stack spacing={1.5} sx={{ width: 1 }}>
          {ratings
            ...
            .reverse()
            .map((rating) => (
              <ProgressLecture key={rating.name} star={rating} total={total} />
            ))}
        </Stack>
      </GridStyle>
...
ProgressLecture.propTypes = {
  star: PropTypes.object,
  total: PropTypes.number,
};

function ProgressLecture({ star, total }) {
  const { name, starCount, reviewCount } = star;
  return (
    <Stack direction="row" alignLectures="center" spacing={1.5}>
