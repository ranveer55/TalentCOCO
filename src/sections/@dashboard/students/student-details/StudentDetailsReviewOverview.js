...
StudentDetailsReviewOverview.propTypes = {
  student: PropTypes.object,
  onOpen: PropTypes.func,
};

export default function StudentDetailsReviewOverview({ student, onOpen }) {
      ...
      <GridStyle student xs={12} md={4}>
        <Stack spacing={1.5} sx={{ width: 1 }}>
          {ratings
            ...
            .reverse()
            .map((rating) => (
              <ProgressStudent key={rating.name} star={rating} total={total} />
            ))}
        </Stack>
      </GridStyle>
...
ProgressStudent.propTypes = {
  star: PropTypes.object,
  total: PropTypes.number,
};

function ProgressStudent({ star, total }) {
