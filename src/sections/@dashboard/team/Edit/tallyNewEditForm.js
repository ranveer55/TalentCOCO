...
TeamNewEditForm.propTypes = {
  isEdit: PropTypes.bool,
  currentTeam: PropTypes.object,
};

export default function TeamNewEditForm({ isEdit, currentTeam }) {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { enqueueSnackbar } = useSnackbar();

  const NewTeamSchema = Yup.object().shape({
    user_id: Yup.number().moreThan(0, 'user_id should not be'),
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      date: currentTeam?.date || '',
      type: currentTeam?.type || '',
      total: currentTeam?.total || '',
      sold: currentTeam?.sold || '',
      broken: currentTeam?.broken || '',
      discount: currentTeam?.discount || '',
      extra: currentTeam?.extra || '',
      salory: currentTeam?.salory || '',
      deposit: currentTeam?.deposit || '',
  ...
  const onSubmit = async () => {
    try {
      dispatch(createTally(defaultValues));
