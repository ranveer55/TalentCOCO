...
TestcaseDetailsReviewOverview.propTypes = {
  testcase: PropTypes.object,
  onOpen: PropTypes.func,
};

export default function TestcaseDetailsReviewOverview({ testcase, onOpen }) {
      ...
      <GridStyle item xs={12} md={4}>
        <Stack spacing={1.5} sx={{ width: 1 }}>
          {ratings
            ...
            .reverse()
            .map((rating) => (
              <ProgressTestcase key={rating.name} star={rating} total={total} />
            ))}
        </Stack>
      </GridStyle>
...
ProgressTestcase.propTypes = {
  star: PropTypes.object,
  total: PropTypes.number,
};

function ProgressTestcase({ star, total }) {
