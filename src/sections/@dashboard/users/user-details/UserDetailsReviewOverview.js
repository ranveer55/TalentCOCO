...
UserDetailsReviewOverview.propTypes = {
  user: PropTypes.object,
  onOpen: PropTypes.func,
};

export default function UserDetailsReviewOverview({ user, onOpen }) {
      ...
      <GridStyle user xs={12} md={4}>
        <Stack spacing={1.5} sx={{ width: 1 }}>
          {ratings
            ...
            .reverse()
            .map((rating) => (
              <ProgressUser key={rating.name} star={rating} total={total} />
            ))}
        </Stack>
      </GridStyle>
...
function ProgressUser({ star, total }) {
